    ...
    <div className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
          ...
          {tabs.map((tab, index) => {
            ...
            return (
              <button
                ...
                className={`
                  group inline-flex items-center gap-3 py-4 px-2 border-b-2 font-medium text-sm transition-all duration-300 relative
                  ${isActive
      ...
      <div className="md:hidden">
        <div className="overflow-x-auto scrollbar-hide">
          <nav className="flex gap-3 px-4 py-3 min-w-max" aria-label="Tabs">
            {tabs.map((tab, index) => {
              const config = tabConfig[tab.id]
              const Icon = config?.icon || User
              const isActive = activeTab === tab.id
              ...
              return (
                <button
                  ...
                  onClick={() => onTabChange(tab.id)}
                  className={`
                    flex flex-col items-center gap-1 px-4 py-3 rounded-xl font-medium text-xs transition-all duration-300 whitespace-nowrap transform
                    ${isActive
